...
def proto(outfile, kind, size, statecount, transkernel):
    ...
    hmm = hmmdef.HMM('test', kind, states, transitions)
...
if __name__ == '__main__':
    ...
    parser.add_argument('targetkind', nargs='?', default='MFCC_0_D_N_Z', type=str)
    parser.add_argument('vecsize', nargs='?', default=25, type=int)
    parser.add_argument('states', nargs='?', default=3, type=int)
    parser.add_argument('transkernel', nargs='+', default=[0.6,0.4], type=float)
    args = parser.parse_args()
    proto(args.outfile, args.targetkind, args.vecsize, args.states, args.transkernel)
